...
shots:
	@if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then \
		...
		docker compose run --rm shots --output docs/assets --base http://api:8000; \
	else \
		echo "[shots] docker unavailable, using offline generator"; \
		python scripts/offline_screens.py; \
	fi
